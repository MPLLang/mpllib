...
struct
  ...
  fun flatSubseqIdxs s (i, j) =
    ...
    | Delay (start, _, f) => Delay (start + i, start + j, f)
  ...
  fun flatIterateIdx (g: 'b * (int * 'a) -> 'b) (b: 'b) s =
    ...
    | Delay (i, j, f) => SeqBasis.foldl g b (i, j) (fn k => (k - i, f k))
  ...
  fun reduceG newGran g b s =
    ...
    | Flat (Delay (i, j, f)) => SeqBasis.reduce newGran g b (i, j) f
    | Nest (n, getBlock) =>
        let
          val nb = numBlocks n
          fun len i =
  ...
  fun mapOption1 (f: 'a -> 'b option) (s: 'a seq) : 'b seq =
    let
      ...
      val offsets =
        SeqBasis.scan 10000 op+ 0 (0, A.length counts) (A.nth counts)
    ...
    case s of
      Flat (Full slice) =>
        Flat
          (Full
             (AS.full
                (SeqBasis.filter gran (0, AS.length slice) (* range *)
                   (AS.nth slice) (* index lookup *)
                   (fn k => p (k, AS.nth slice k)) (* index predicate *))))

    | Flat (Delay (i, j, f)) =>
        Flat (Full (AS.full (SeqBasis.filter gran (i, j) f (fn k =>
          p (k, f k)))))
  ...
  fun scanDelay g b (i, j, f) =
    let
      ...
      val blockSums = SeqBasis.tabulate 1 (0, nb) (fn blockIdx =>
        let
      ...
      val partials = SeqBasis.scan gran g b (0, nb) (A.nth blockSums)
  ...
  fun scanScan g b (n, getChild: int -> 'a Stream.t) =
    let
      ...
      val childSums = SeqBasis.tabulate 1 (0, numChildren) (fn childIdx =>
        Stream.iterate g b (len childIdx, getChild childIdx))

      val partials = SeqBasis.scan gran g b (0, numChildren) (A.nth childSums)

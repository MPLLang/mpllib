...
sig
  ...
  val doSplit:
    ...
    -> vertex
       * Geometry2D.point (* point inside triangle, and vertex identifier to use *)
...
struct
  ...
  val dummyTri = Tri {vertices = dummyTriple, neighbors = dummyTriple}
  ...
  fun new {numVertices, numTriangles} =
    Mesh
      { vdata = Seq.tabulate (fn _ => dummyPt) numVertices
  ...
  fun copyData {src = Mesh src, dst = Mesh dst} =
    let
      val len = Seq.length
      val lengthsOkay =
        len (#vdata src) <= len (#vdata dst)
        andalso len (#triangleOfVertex src) <= len (#triangleOfVertex dst)
        andalso len (#verticesOfTriangle src) <= len (#verticesOfTriangle dst)
        andalso len (#neighborsOfTriangle src) <= len (#neighborsOfTriangle dst)
      val _ =
        if lengthsOkay then ()
  ...
  fun tdata (Mesh mesh) t =
    Tri
      { vertices = nth (#verticesOfTriangle mesh) t
  ...
  fun vdata (Mesh mesh) t =
    nth (#vdata mesh) t

  fun triangleOfVertex (Mesh mesh) v =
    nth (#triangleOfVertex mesh) v
  ...
  fun hasEdge (a, b, c) (u, v) =
    (u = a orelse u = b orelse u = c) andalso (v = a orelse v = b orelse v = c)


  fun sortTriangleCCW mesh
    (Tri {vertices = (v1, v2, v3), neighbors = (t1, t2, t3)}) =
    let
      fun p v = vdata mesh v
      val (v2, v3) =
        if G.Point.counterClockwise (p v1, p v2, p v3) then (v2, v3)
        else (v3, v2)
      ...
      val (t1, t2, t3) =
        if checkHasEdge (v1, v3) t1 then (t1, t2, t3)
        else if checkHasEdge (v1, v3) t2 then (t2, t1, t3)
        else (t3, t1, t2)

      val (t2, t3) = if checkHasEdge (v2, v1) t2 then (t2, t3) else (t3, t2)
  ...
  fun orientedTriangleData mesh (t, i) =
    let
      val Tri {vertices = (a, b, c), neighbors = (d, e, f)} = tdata mesh t
    in
      case i of
  ...
  fun fastLocate (a, b, _) (t: triangle) =
    if a = t then 0 else if b = t then 1 else 2

  fun fastAcross mesh ((t, i): simplex) =
    let val t' = fastNeighbor (neighborsOfTriangle mesh t) i
    in (t', fastLocate (neighborsOfTriangle mesh t') t)
    end
  ...
  fun findPoint mesh pt current =
    if pointOutside mesh current pt then
      ...
      let
        val current = rotateClockwise current
      in
        if pointOutside mesh current pt then
          findPoint mesh pt (fastAcross mesh current)
        else
  ...
  fun updateTriangle (Mesh {verticesOfTriangle, neighborsOfTriangle, ...}) t
    (Tri {vertices, neighbors}) =
    (upd verticesOfTriangle t vertices; upd neighborsOfTriangle t neighbors)
  ...
  fun doSplit (mesh as Mesh {vdata, triangleOfVertex, ...}) t (v, p) (ta0, ta1) =
    let
      ...
      val newdata_t = Tri {vertices = (v1, v, v3), neighbors = (t1, ta0, ta1)}
      val newdata_ta0 = Tri {vertices = (v2, v, v1), neighbors = (t2, ta1, t)}
      val newdata_ta1 = Tri {vertices = (v3, v, v2), neighbors = (t3, t, ta0)}
    in
      upd vdata v p;
      upd triangleOfVertex v t;
      if nth triangleOfVertex v2 <> t then () else upd triangleOfVertex v2 ta0;
  ...
  fun split
    (Mesh {verticesOfTriangle, neighborsOfTriangle, vdata, triangleOfVertex})
    (t: triangle) p =
    let
      ...
      val verticesOfTriangle' = Seq.append
        (verticesOfTriangle, Seq.fromList [dummyTriple, dummyTriple])
      val neighborsOfTriangle' = Seq.append
        (neighborsOfTriangle, Seq.fromList [dummyTriple, dummyTriple])
  ...
  fun doFlip (mesh as Mesh {triangleOfVertex, ...}) (simp: simplex) =
    let
      ...
      val newdata_t = Tri {vertices = (v2, v3, v4), neighbors = (t1, t3, t4)}

      val newdata_t1 = Tri {vertices = (v1, v2, v4), neighbors = (t5, t2, t)}

      fun replaceTriangleOfVertex v (old, new) =
        if nth triangleOfVertex v <> old then () else upd triangleOfVertex v new
    in
      updateTriangle mesh t newdata_t;
  ...
  fun flip
    ...
    let
      (** make a copy *)
      val mesh' = Mesh
        { verticesOfTriangle = Seq.map (fn x => x) verticesOfTriangle
  ...
  *)
  ...
  fun ripAndTentOne ((t, simps): cavity, pt: G.point) mesh =
    ...
    let val mesh' = split mesh t pt
    in List.app (doFlip mesh') simps; mesh'
    end


  fun ripAndTent cavities
    (Mesh {verticesOfTriangle, neighborsOfTriangle, vdata, triangleOfVertex}) =
    let
      ...
      val vdata' =
        Seq.tabulate (fn i => if i < numVerts then nth vdata i else dummyPt)
          (numVerts + numNewVerts)
      val verticesOfTriangle' =
        Seq.tabulate
          (fn i =>
             if i < numTriangles then nth verticesOfTriangle i else dummyTriple)
          (numTriangles + numNewTriangles)
      val neighborsOfTriangle' =
        Seq.tabulate
          (fn i =>
             if i < numTriangles then nth neighborsOfTriangle i else dummyTriple)
          (numTriangles + numNewTriangles)
      val triangleOfVertex' =
        Seq.tabulate
          (fn i => if i < numVerts then nth triangleOfVertex i else INVALID_ID)
          (numVerts + numNewVerts)
      val mesh' = Mesh
        { vdata = vdata'
        , verticesOfTriangle = verticesOfTriangle'
  ...
  fun initialMeshWithBoundaryCircle {numVertices, numBoundaryVertices}
    {center, radius} =
    let
      val pi = Math.pi
      ...
      fun verticesOfTriangle i =
        if i < numNonBoundaryTriangles then
          ...
          let val j = i - numNonBoundaryTriangles + numNonBoundaryVertices
          in (j + 1, j + 2, numNonBoundaryVertices)
          end
    ...
    let
      fun loop old =
        if x <= old then
          ...
          let val old' = Concurrency.casArray (a, i) (old, x)
          in if old' = old then () else loop old'
          end
    in
      loop (Array.sub (a, i))
    end
    ...
    let
      ...
      fun tryParse parser test thing lineNum =
        let
          fun whoops msg =
            raise Fail
              ("Topology2D: line " ^ Int.toString (lineNum + 1)
        ...
        in
          case
            (parser (line lineNum) handle exn => whoops (SOME (exnMessage exn)))
          of
      ...
      fun validVid x =
        (0 <= x andalso x < numVertices)
      fun validTid x =
        (x = INVALID_ID orelse (0 <= x andalso x < numTriangles))
      fun validTriangle (Tri {vertices = (a, b, c), neighbors = (d, e, f)}) =
        validVid a andalso validVid b andalso validVid c andalso validTid d
        andalso validTid e andalso validTid f
      ...
      fun neighborsParser restOfLine =
        let
          ...
          fun isSpace i =
            Char.isSpace (Seq.nth restOfLine i)
          ...
          fun nbrEnd i =
            if i = Seq.length spPos then Seq.length restOfLine
            else Seq.nth spPos i
          fun nbr i =
            if i >= numNbrs then
              INVALID_ID
            else
      ...
      val _ =
        if numLines >= numVertices + numTriangles + 3 then
          ...
          raise Fail "Topology2D: not enough vertices and/or triangles to parse"

      val vertices = AS.full (SeqBasis.tabulate 1000 (0, numVertices) (fn i =>
        tryParseVertex (3 + i)))

      val verticesOfTriangle = AS.full (ForkJoin.alloc numTriangles)
      val neighborsOfTriangle = AS.full (ForkJoin.alloc numTriangles)
      val triangleOfVertex = ForkJoin.alloc numVertices
      val _ = ForkJoin.parfor 10000 (0, numVertices) (fn i =>
        Array.update (triangleOfVertex, i, ~1))

      val _ = ForkJoin.parfor 1000 (0, numTriangles) (fn i =>
        let
          val Tri {vertices = (a, b, c), neighbors} = tryParseTriangle
            (3 + numVertices + i)
        in
  ...
  fun toString (mesh as Mesh {vdata = vertices, ...}) =
    ...
    in
      String.concatWith "\n"
        (["Mesh", Int.toString nv, Int.toString nt]

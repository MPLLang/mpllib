      ...
      let
        ...
        val ((sorted, offsets), _) = Util.getTime (fn () =>
          CountingSort.sort idx
            (fn i => G.quadrant center (Seq.nth verts (Seq.nth idx i))) 4)
  ...
  fun reduce grain f b (get, lo, hi) =
    if hi - lo <= grain then
      ...
      let
        ...
        val (l, r) =
          ForkJoin.par (fn _ => reduce grain f b (get, lo, mid), fn _ =>
            reduce grain f b (get, mid, hi))
      in
        f (l, r)
      end
  ...
  fun makeTree leafSize (verts: point seq) =
    if AS.length verts = 0 then
      ...
      let
        ...
        val (xLeft, yBot) =
          reduce 10000 minPt (Real.posInf, Real.posInf)
            (getPt, 0, AS.length verts)
        val (xRight, yTop) =
          reduce 10000 maxPt (Real.negInf, Real.negInf)
            (getPt, 0, AS.length verts)

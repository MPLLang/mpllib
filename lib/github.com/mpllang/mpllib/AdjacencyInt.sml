...
struct
      ...
      fun processVertex v =
        if not (h v) then
          ...
          let
            ...
            fun loop i =
              if i >= Seq.length neighbors then
                0
              else
                let
                  val u = Seq.nth neighbors i
                in
  ...
  fun edge_map_sparse g vertices f h =
    let
      ...
      val r = SeqBasis.scan 1000 op+ 0 (0, n) (G.degree g o ui)
      ...
      fun map_block i =
        let
          ...
          val ulo = let val a = BinarySearch.search Int.compare offsets lo
                    in if (Seq.nth offsets a) > lo then a - 1 else a
                    end
          fun map_seq idx (u, uidx) count =
            if idx >= hi then
              count
            else if idx >= (Seq.nth offsets (uidx + 1)) then
              map_seq idx (ui (uidx + 1), uidx + 1) count
            else
              let
              ...
              in
                if (h v) then
                  case f (u, v) of
                    SOME x =>
                      ( Array.update (store, lo + count, x)
                      ; map_seq (idx + 1) (u, uidx) (count + 1)
                      )
      ...
      val outOff = SeqBasis.scan 10000 op+ 0 (0, numBlocks) (fn i =>
        Array.sub (counts, i))
      val outSize = Array.sub (outOff, numBlocks)
      val result = ForkJoin.alloc outSize
    in
      ForkJoin.parfor (totalOutDegree div (Int.max (outSize, 1))) (0, numBlocks)
        (fn i =>
           let
             val soff = i * k
  ...
  fun edge_map g (vs, threshold) (fpar, f) h =
    ...
    | DENSE s =>
        let val (res, count) = edge_map_dense g s f h
        in from_dense_rep res (SOME count) threshold
        end

  fun vertex_foreach _ (vs, _) f =
    case vs of
      SPARSE s => Seq.foreach s (fn (_, u) => f u)
    | DENSE s => Seq.foreach s (fn (i, b) => if (b = 1) then (f i) else ())

  fun vertex_map_ g (vs, threshold) f =
    case vs of
      SPARSE s =>
        let
          val s' = AS.full (SeqBasis.tabFilter 1000 (0, Seq.length s) (fn i =>
            let
              val u = Seq.nth s i
              val b = f u
            in
  ...
  fun vertex_map g vs f needOut =
    if needOut then vertex_map_ g vs f else (vertex_foreach g vs; vs)

  ...
  fun seqSortInternal In Out Keys Counts genOffsets =
    ...
    in
      ...
      for (0, n) (fn i =>
        ...
        in update (Counts, j, sub (Counts, j) + 1)
        end);

      (* print ("counts: " ^ Seq.toString Int.toString Counts ^ "\n"); *)

      loop (0, m) 0 (fn (s, i) =>
        let val t = sub (Counts, i)
        in update (Counts, i, s); s + t
        end);
    ...
    in
      if (numBlocks <= 1 orelse n < SeqThreshold) then
        ...
        let
          ...
          val _ = ForkJoin.parfor 1 (0, numBlocks) (fn i =>
            ...
            in
              seqSortInternal (AS.subslice (In, start, SOME len))
                (AS.subslice (B, start, SOME len)) (fn i => Keys (i + start))
          ...
          val C = SampleSort.transposeBlocks
            (B, sourceOffsets, destOffsets, Counts, numBlocks, numBucketsPad, n)
          val bucketOffsets =
            Seq.tabulate
              (fn i =>
                 if (i = numBuckets) then n
                 else AS.sub (destOffsets, i * numBlocks)) (numBuckets + 1)
        in
          (C, bucketOffsets)
        end
    end

...
struct
  ...
  fun sortRange grainsize (array, start, n, compare) =
    let
      ...
      fun rotate (i, j, k) =
        let val tmp = item k
        in set (k, item j); set (j, item i); set (i, tmp)
        end
                ...
                fun inner j =
                  if j = start then
                    ...
                    let
                      val j' = j - 1
                    in
                      if cmp (j', j) = GREATER then (swap (j, j'); inner j')
                      else outer (i + 1)
                    end
              in
                inner i
      ...
      fun split3 (a, n) =
        let
          ...
          fun right r =
            if cmp (r, p2) = GREATER then right (r - 1) else r
          fun loop (l, m, r) =
            if (m > r) then
              (l, m)
          ...
          in
            if (n <= grainsize) then
              ...
              let
                ...
                val maybeMid =
                  if doMid then (fn () => (par (mid, right); ())) else right
              in
                par (left, maybeMid);
                ()
              end

...
struct
  ...
  fun toString t =
    ...
    | Chunk arr =>
        "("
        ^
        String.concatWith " " (List.tabulate (A.length arr, fn i =>
          Key.toString (#1 (A.sub (arr, i))))) ^ ")"
    | Node {left, key, right, ...} =>
  ...
  fun makeChunk arr {start, len} =
    if len = 0 then Empty else Chunk (A.subseq arr {start = start, len = len})
  ...
  fun makeNode left (k, v) right =
    ...
    in
      if n <= leafSize then Chunk (A.tabulate (n, nth node)) else node
    end
  ...
  in
    fun updateKey t (k, v) =
      ( (*print ("updateKey " ^ toString t ^ " (" ^ Key.toString k ^ ", ...)" ^ "\n");*)
        case t of
          Empty => fail k
            ...
            let
              ...
              fun loop i =
                if i >= n then
                  ...
                  case Key.compare (k, #1 (A.sub (arr, i))) of

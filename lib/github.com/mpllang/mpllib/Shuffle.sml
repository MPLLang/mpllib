...
struct
  ...
  fun inplace_seq_shuffle s l r seed =
    let
      ...
      fun rand_idx i =
        Int.mod (Util.hash (seed + i), i - l + 1) + l
      fun swap (i, j) =
        let val tmp = item i
        in set (i, item j); set (j, tmp)
        end
  ...
  fun bucket_shuffle s seed =
    let
      fun log2_up n =
        Real.ceil (Math.log10 (Real.fromInt n) / (Math.log10 2.0))
      fun bit_and (n, mask) =
        Word.toInt (Word.andb (Word.fromInt n, mask))

      val n = length s
      val l = log2_up n
      val bits =
        if n < Real.floor (Math.pow (2.0, 27.0)) then Int.div ((l - 7), 2)
        else l - 17
      val num_buckets = Real.floor (Math.pow (2.0, Real.fromInt bits))
      val mask = Word.fromInt (num_buckets - 1)
      fun rand_pos i =
        bit_and (Util.hash (seed + i), mask)
      (* size of bucket_offsets = num_buckets + 1 *)
      val (s', bucket_offsets) = CountingSort.sort s rand_pos num_buckets
      fun bucket_shuffle i =
        inplace_seq_shuffle s' (nth bucket_offsets i)
          (nth bucket_offsets (i + 1)) seed

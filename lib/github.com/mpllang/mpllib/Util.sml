...
struct
    ...
    let
      ...
      fun loop q i =
        let
          ...
          val q = Int.quot (q, 10)
        in
          if q <> 0 then loop q i
          else if orig < 0 then (Array.update (buf, i - 1, #"~"); i - 1)
          else i
        end

      val start = loop (if orig < 0 then orig else ~orig) maxNumChars
    in
      CharVector.tabulate (maxNumChars - start, fn i =>
        Array.sub (buf, start + i))
    end
  ...
  fun reportTime f =
    let val (result, tm) = getTime f
    in print ("time " ^ Time.fmt 4 tm ^ "s\n"); result
    end
  ...
  fun searchPow2 n m =
    if m >= n then m else searchPow2 n (2 * m)
  ...
  fun foreach s f =
    ForkJoin.parfor 4096 (0, ArraySlice.length s) (fn i =>
      f (i, ArraySlice.sub (s, i)))

...
sig
  ...
  val reduce: grain -> ('a * 'a -> 'a) -> 'a -> (int * int) -> (int -> 'a) -> 'a
  ...
  val filter: grain -> (int * int) -> (int -> 'a) -> (int -> bool) -> 'a array
...
struct
  ...
  *)
  ...
  fun tabulate grain (lo, hi) f =
    ...
    in
      if lo = 0 then parfor grain (0, n) (fn i => upd result i (f i))
      else parfor grain (0, n) (fn i => upd result i (f (lo + i)));

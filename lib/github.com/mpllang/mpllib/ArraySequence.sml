...
struct
  ...
  fun scan f b s =
    let val p = AS.full (SeqBasis.scan GRAN f b (0, length s) (nth s))
  ...
  fun filterSafe p s =
    ...
    AS.full (SeqBasis.tabFilter GRAN (0, length s) (fn i =>
      if p (nth s i) then SOME (nth s i) else NONE))

  fun filterIdx p s =
    AS.full (SeqBasis.filter GRAN (0, length s) (nth s) (fn i =>
      p (i, nth s i)))

  fun filtermap (p: 'a -> bool) (f: 'a -> 'b) (s: 'a t) : 'b t =
    AS.full (SeqBasis.filter GRAN (0, length s) (fn i => f (nth s i))
      (p o nth s))
  ...
  fun equal eq (s, t) =
    length s = length t
    andalso
    SeqBasis.reduce GRAN (fn (a, b) => a andalso b) true (0, length s) (fn i =>
      eq (nth s i, nth t i))

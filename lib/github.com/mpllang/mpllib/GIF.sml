...
struct
  ...
  fun ceilLog2 n =
    if n <= 0 then err "ceilLog2: expected input at least 1"
    else (* Util.log2(x) computes 1 + floor(log_2(x)) *) Util.log2 (n - 1)
  ...
  struct
    ...
    fun makeQuantized (rqq, gqq, bqq) =
      let
        ...
        fun bucketShift numBuckets =
          Word8.fromInt
            ((255 - (numBuckets - 1) * (bucketSize numBuckets)) div 2)

        fun qi nb =
          fn c => Word8.toInt c div (bucketSize nb)
        fun qc nb =
          fn i => bucketShift nb + Word8.fromInt (i * bucketSize nb)

        fun makeQ nb =
          {numBuckets = nb, channelToIdx = qi nb, channelFromIdx = qc nb}

        val R = makeQ rqq
        val G = makeQ gqq
        val B = makeQ bqq
        val numQuantized = (* this should be at most 256 *) List.foldl op* 1
          (List.map #numBuckets [R, G, B])

        fun channelIndices {red, green, blue} =
          (#channelToIdx R red, #channelToIdx G green, #channelToIdx B blue)

        fun packChannelIndices (r, g, b) =
          b + g * (#numBuckets B) + r * (#numBuckets B) * (#numBuckets G)
        ...
        fun colorOfQuantizeIdx i =
          let
            ...
            val r =
              (i div (#numBuckets B) div (#numBuckets G)) mod (#numBuckets R)
          in
    ...
    fun summarizeBySampling requiredColors paletteSize
      (sample: int -> Color.pixel) =
      if paletteSize <= 0 then
        ...
        let
              ...
              Util.loop (bounds r) (valOf Int.maxInt) (fn (md, r) =>
                Util.loop (bounds g) md (fn (md, g) =>
                  Util.loop (bounds b) md (fn (md, b) =>
                    List.foldl (fn (c, md) => Int.min (md, dist (c, color))) md
          ...
          val bucketOffsets = AS.full
            (SeqBasis.scan 100 op+ 0 (0, numBuckets) (Seq.nth bucketSizes))
          val palette = ForkJoin.alloc paletteSize
          val _ = Util.for (0, numBuckets) (fn i =>
            ignore (Util.copyListIntoArray (Seq.nth buckets i) palette
  ...
  struct
    ...
    type t = {nextCode: int ref, numColors: int, table: (idx * code) list array}
    ...
    fun insert (code, idx) ({nextCode, numColors = _, table}: t) =
      ...
      in
        if ret then
          ( Array.update
              (table, code, (idx, !nextCode) :: Array.sub (table, code))
          ; nextCode := !nextCode + 1
          )
        else
          ();
  ...
  *)
  ...
  struct
    ...
    fun codeStream numColors colorIndices =
      ...
      in
        if Seq.length colorIndices = 0 then err "empty color index sequence"
        else loop (DS.push clear (DS.new ())) (colorIdx 0) 1
      end
    ...
    fun packCodeStream numColors codes =
      let
        ...
        val clears = AS.full (SeqBasis.filter 2000 (0, n) (fn i => i) (fn i =>
          code i = clear))
          ...
          in
            Util.loop (i, j) (firstCodeWidth, Util.pow2 firstCodeWidth)
              (fn ((currWidth, cwPow2), k) =>
                 ...
                 ; if currentMaxCode (k + 1) <> cwPow2 then (currWidth, cwPow2)
                   else (currWidth + 1, cwPow2 * 2)
                 ));
            ()
          end)
        ...
        ForkJoin.parfor 10 (0, numBlocks) (fn i =>
          ...
          in
            ...
            Util.for (0, size) (fn j =>
              Array.update (output, 256 * i + 1 + j, Seq.nth packed
                (255 * i + j)))
          end);
  ...
  fun checkToWord16 thing x =
    if x >= 0 andalso x <= 65535 then Word16.fromInt x
    else err (thing ^ " must be non-negative and less than 2^16")
  ...
  fun packScreenDescriptorByte
    ...
    in
      ...
      orb ((fromInt colorResolution andb 0wx7) << 0w4)
      orb ((if sortFlag then 0w1 else 0w0) << 0w3)
      orb (fromInt colorTableSize andb 0wx7)
    end

  fun writeMany path delay ({colors, ...}: Palette.t)
    {width, height, numImages, getImage} =
    if numImages <= 0 then
      ...
      in
        ...
        Util.for (0, numberOfColors) (fn i => wrgb (Seq.nth colors i));
        ...
        Util.for (0, numImages) (fn i =>
          ...
          in
            ...
            Util.for (0, Seq.length bytes) (fn i => w8 (Seq.nth bytes i))
          end);

...
sig
  ...
  datatype am =
    Leaf
  | Node of {l: am, k: T.key, v: T.value, a: T.aug, r: am, size: int}
  val empty: unit -> am
  val size: am -> int
...
struct
  ...
  datatype am =
    Leaf
  | Node of {l: am, k: key, v: value, a: aug, r: am, size: int}
  exception Assert
  structure T = T
  ...
  struct
    ...
    fun heavy (m1: am, m2: am) =
      size_heavy (weight m1) (weight m2)

    fun like s1 s2 =
      not (size_heavy s1 s2) andalso not (size_heavy s2 s1)
    ...
    fun rotateLeft m =
      ...
      | Node {l, k, v, a = _, r, size = _} =>
          ...
          | Node {l = rl, k = rk, v = rv, r = rr, ...} =>
              let val left = compose l k v rl
              in compose left rk rv rr
              end
    ...
    fun rotateRight m =
      ...
      | Node {l, k, v, a = _, r, size = _} =>
          ...
          | Node {l = ll, k = lk, v = lv, r = lr, ...} =>
              let val right = compose lr k v r
              in compose ll lk lv right
              end
  ...
  fun eval (inpar, f1, f2) =
    if inpar then ForkJoin.par (f1, f2) else (f1 (), f2 ())

  fun join m1 k v m2 =
    let val WB _ = T.balance
    in WBST.join m1 k v m2
    end
  ...
  fun join2 m1 m2 =
    ...
    in
      case m1 of
        Leaf => m2
      | Node n =>
          let val (m', k', v') = splitLast n (*get the greatest element in m1*)
          in join m' k' v' m2
          end
    end
  ...
  fun build_sorted s i j =
    if i = j then
      ...
      singleton (#1 (Seq.nth s i)) (#2 (Seq.nth s i))
    else
      let
        val m = i + Int.div ((j - i), 2)
        val (l, r) =
          if (j - i) > 1000 then
            eval (true, fn _ => build_sorted s i m, fn _ =>
              build_sorted s (m + 1) j)
          else
            (build_sorted s i m, build_sorted s (m + 1) j)
        val (x, y) = Seq.nth s m
      in
        join l x y r
      end
    ...
    let
      ...
      fun insert_helper m' i j =
        if i >= j then
          ...
          SeqBasis.foldl (fn (m'', (k, v)) => insert m'' k v h) m' (i, j)
            (Seq.nth ss)
        else
          case m' of
                  ...
                  if (i + 1 = j) then
                    ...
                    in
                      ...
                      | EQUAL =>
                          let
                            val bound_func =
                              until_boundary i j (fn t => key_equal k (#1 t))
                                mid
                          in
                            ( bound_func (fn i => i - 1)
                            , 1 + bound_func (fn i => i + 1)
                            )
                          end
                      | GREATER => bin_search k mid j
                    end
                val (lk, rk) = bin_search kr i j
                val (l, r) = par (fn _ => insert_helper l i lk, fn _ =>
                  insert_helper r rk j)
                val nvr =
                  SeqBasis.foldl (fn (v', (_, v)) => h (v, v')) vr (lk, rk)
                    (Seq.nth ss)
              in
                join l kr nvr r
              end
  ...
  fun split m k =
    ...
    | Node {l, k = kr, v = vr, r, ...} =>
        case T.compare (k, kr) of
          EQUAL => (l, SOME vr, r)
        | LESS =>
            let val (ll, so, lr) = split l k
            in (ll, so, join lr kr vr r)
            end
        | GREATER =>
            let val (rl, so, rr) = split r k
            in (join l kr vr rl, so, rr)
            end
  ...
  fun union m1 m2 h =
    ...
    | (Node {l = l1, k = k1, v = v1, r = r1, size, ...}, m2) =>
        let
          ...
          val (ul, ur) = eval (size > gran, fn _ => union l1 l2 h, fn _ =>
            union r1 r2 h)
        in
  ...
  fun filter h m =
    case m of
      Leaf => m
    | Node {l, k, v, r, ...} =>
        let val (l', r') = par (fn _ => filter h l, fn _ => filter h r)
        in if h (k, v) then join l' k v r' else join2 l' r'
        end
  ...
  fun mapReduce m g f id =
    ...
    | Node {l, k, v, r, size, ...} =>
        let
          val (l', r') = eval (size > gran, fn _ => mapReduce l g f id, fn _ =>
            mapReduce r g f id)
        in
          f (f (l', g (k, v)), r')
        end
  ...
  fun build ss i _ =
    let
      ...
      fun cmp (x, y) =
        T.compare (#1 (Seq.nth ss x), #1 (Seq.nth ss y))
      ...
      val t3 = Time.now ()
      val _ = print
        ("from sorted time = " ^ Time.fmt 4 (Time.- (t3, t2)) ^ "s\n")
    in
      r
    end
  ...
  fun aug_filter m h =
    ...
    | Node {l, k, v, r, size, ...} =>
        let
          val (l', r') = eval (size > gran, fn _ => aug_filter l h, fn _ =>
            aug_filter r h)
        in
          if h (T.g (k, v)) then join l' k v r' else join2 l' r'
        end
  ...
  fun aug_project (ga: T.aug -> 'a) fa m (k1: T.key) (k2: T.key) =
    let
      ...
      fun compose_map_kv m k v =
        fa (ga (aug_val m), ga (T.g (k, v)))

      fun compose_kv_map k v m =
        fa (ga (T.g (k, v)), ga (aug_val m))
  ...
  fun aug_range m k1 k2 =
    aug_project (fn x => x) T.f m k1 k2

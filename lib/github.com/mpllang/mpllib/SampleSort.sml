...
sig
  ...
  val transposeBlocks: 'a seq * int seq * int seq * int seq * int * int * int
                       -> 'a seq
...
struct
    ...
    let
      fun r (rs, rl, cs, cl) =
        if (rl * cl < threshold) then
          ...
          ( par (fn () => r (rs, rl, cs, cl div 2), fn () =>
              r (rs, rl, cs + (cl div 2), cl - (cl div 2)))
          ; ()
          )
        else
          ( par (fn () => r (rs, rl div 2, cs, cl), fn () =>
              r (rs + (rl div 2), rl - (rl div 2), cs, cl))
          ; ()
          )
    in
  ...
  fun transposeBlocks
    (S, source_offsets, dest_offsets, counts, num_rows, num_cols, _) =
    let
      ...
      fun baseCase (row_start, row_len, col_start, col_len) =
        for (row_start, row_start + row_len) (fn i =>
          for (col_start, col_start + col_len) (fn j =>
            let
    ...
    let
      ...
      fun merge (i, j) =
        if (j = num_samples) then
          ...
          let
            fun merge' i =
              if (i < n andalso cmp (AS.sub (A, i), AS.sub (S, j)) = LESS) then
                merge' (i + 1)
              else
                i
            val k = merge' i
  ...
  fun sort cmp A =
    let
      ...
      val C = transposeBlocks
        (B, source_offsets, dest_offsets, counts, num_blocks, num_buckets, n)
      ...
      fun bucket_offset i =
        if (i = num_buckets) then n else AS.sub (dest_offsets, i * num_blocks)

      (* sort the buckets *)
      val _ = parallelFor 1 (0, num_buckets) (fn i =>
        let

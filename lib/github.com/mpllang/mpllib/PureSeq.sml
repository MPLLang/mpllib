...
struct
  ...
  fun empty () =
    VS.full (V.fromList [])
  fun fromList xs =
    VS.full (V.fromList xs)

  fun subseq s (i, n) =
    VS.subslice (s, i, SOME n)
  ...
  fun filterIdx p s =
    VS.full (unsafeCast (SeqBasis.filter gran (0, length s) (nth s) (fn i =>
      p (i, nth s i))))
  ...
  fun writeMerge cmp (s1: 'a seq, s2: 'a seq) (t: 'a AS.slice) =
    if AS.length t <= gran then
      ...
      in
        ForkJoin.par (fn _ => writeMerge cmp (l1, l2) tl, fn _ =>
          writeMerge cmp (r1, r2) tr);
        ()
      end

  fun merge cmp (s1, s2) =
    let val out = ForkJoin.alloc (length s1 + length s2)
    in writeMerge cmp (s1, s2) (AS.full out); VS.full (unsafeCast out)
    end

  fun quicksort cmp s =
    let val out = SeqBasis.tabulate gran (0, length s) (nth s)
    in Quicksort.sortInPlace cmp (AS.full out); VS.full (unsafeCast out)
    end
  ...
  fun summarize count toString xs =
    let
      ...
      val strs =
        if count <= 0 then
          raise Fail "PureSeq.summarize needs count > 0"
